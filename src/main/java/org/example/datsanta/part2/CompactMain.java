...
//        System.out.println(types);
        ...
        while (!children.isEmpty()) {
            ...
            final Gift2 gift = isFirstFull
                    ? getMinPriceGift(gifts, giftTypes)
                    : getMaxPriceGift(gifts, giftTypes);
            gifts.remove(gift);
    ...
    private static Gift2 getMinPriceGift(final List<Gift2> gifts, final List<GiftType> giftTypes) {
        return gifts.stream()
                .filter(e -> giftTypes.contains(e.getType()))
